...
const char * const BITCOIN_CONF_FILENAME = "proinvestcoin.conf";

bool fProInvestCoinMode = true;
bool fProInvestCoinWallet = false;
ArgsManager gArgs;
    ...
    for (const auto& arg_map : m_available_args) {
        switch(arg_map.first) {
            ...
            case OptionsCategory::SMSG:
                ...
                break;
            case OptionsCategory::PIC_WALLET:
                usage += HelpMessageGroup("ProInvestCoin wallet Commands:");
                break;
            case OptionsCategory::PIC_STAKING:
...
#else
    const char* pszModule = "proinvestcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ProInvestCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ProInvestCoin
    // Mac: ~/Library/Application Support/ProInvestCoin
    // Unix: ~/.proinvestcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ProInvestCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ProInvestCoin";
#else
    // Unix
    return pathRet / ".proinvestcoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    const int PIC_START_YEAR = 2017;

    std::string sRange = PIC_START_YEAR == COPYRIGHT_YEAR
        ? strprintf(" %i ", COPYRIGHT_YEAR)
        : strprintf(" %i-%i ", PIC_START_YEAR, COPYRIGHT_YEAR);
