// Copyright (c) 2017-2019 The ProInvestCoin Core developers
...
static int ExtractBip32InfoV(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_SECRET_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "ProInvestCoin extended secret key");
    } else
...
static int ExtractBip32InfoP(const std::vector<uint8_t> &vchKey, UniValue &keyInfo, std::string &sError)
{
    ...
    if (memcmp(&vchKey[0], &Params().Base58Prefix(CChainParams::EXT_PUBLIC_KEY)[0], 4) == 0) {
        keyInfo.pushKV("type", "ProInvestCoin extended public key");
    } else
...
static UniValue extkey(const JSONRPCRequest &request)
{
    ...
    CHDWallet *const pwallet = GetProInvestCoinWallet(wallet.get());
    ...
    if (mode == "import") {
        ...
        if (fBip44) {
            ...
            if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                ...
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Import failed - Key must begin with a proinvestcoin prefix.");
            }
        }
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"extkeyaltversion",
                ...
                "If the provided ext_key has a Bitcoin prefix the output will be encoded with a ProInvestCoin prefix.\n"
                "If the provided ext_key has a ProInvestCoin prefix the output will be encoded with a Bitcoin prefix.\n",
                {
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"getnewextaddress",
                "\nReturns a new ProInvestCoin ext address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                ...
                RPCResult{
            "\"address\"              (string) The new proinvestcoin extended address\n"
                },
                RPCExamples{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{"getnewstealthaddress",
                "\nReturns a new ProInvestCoin stealth address for receiving payments." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"label", RPCArg::Type::STR, /* default */ "", "If specified the key is added to the address book."},
            ...
            "           Stealth addresses with prefixes will scan only incoming stealth transactions with a matching prefix."},
                ...
                RPCResult{
            "\"address\"              (string) The new proinvestcoin stealth address\n"
                },
                RPCExamples{
...
static UniValue manageaddressbook(const JSONRPCRequest &request)
{
    ...
    if (!address.IsValid()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, _("Invalid ProInvestCoin address."));
    }
...
static UniValue getstakinginfo(const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"moneysupply\": xxxxxxx,        (numeric) the total amount of proinvestcoin in the network\n"
            "  \"reserve\": xxxxxxx,            (numeric) the total amount of proinvestcoin in the network\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspentanon",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "", "A json array of proinvestcoin addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, /* default */ "", "proinvestcoin address"},
                        },
                    },
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValidStealthAddress())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid ProInvestCoin stealth address: ")+input.get_str());
    ...
    if (request.params[0].isArray()) {
        ...
        for (size_t k = 0; k < outputs.size(); ++k) {
            ...
            if (typeOut == OUTPUT_RINGCT
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ProInvestCoin stealth address");
            }
...
static std::string SendHelp(CHDWallet *pwallet, OutputTypes typeIn, OutputTypes typeOut)
{
    ...
    rv += std::string(" pic in a") + (typeOut == OUTPUT_RINGCT || typeOut == OUTPUT_CT ? " blinded" : "") + " payment to a given address"
        + (typeOut == OUTPUT_CT ? " in anon pic": "") + ".\n";

    rv += HelpRequiringPassphrase(pwallet);

    rv +=   "\nArguments:\n"
            "1. \"address\"     (string, required) The proinvestcoin address to send to.\n"
...
static UniValue sendpictoblind(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetProInvestCoinWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendpictoanon(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetProInvestCoinWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendblindtopic(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetProInvestCoinWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
...
static UniValue sendanontopic(const JSONRPCRequest &request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CHDWallet *const pwallet = GetProInvestCoinWallet(wallet.get());
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp))
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
            RPCHelpMan{"sendtypeto",
                "\nSend pic to multiple outputs." +
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"typein", RPCArg::Type::STR, RPCArg::Optional::NO, "pic/blind/anon"},
                    {"typeout", RPCArg::Type::STR, RPCArg::Optional::NO, "pic/blind/anon"},
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of json objects",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::NO, "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The proinvestcoin address to send to."},
            ...
            "                             transaction, just kept in your wallet."},
                    ...
                    {"coin_control", RPCArg::Type::OBJ, /* default */ "", "",
                        {
                            {"changeaddress", RPCArg::Type::STR, /* default */ "", "The proinvestcoin address to receive the change"},
            ...
            "\"txid\"              (string) The transaction id.\n"
                ...
                RPCExamples{
            HelpExampleCli("sendtypeto", "anon pic \"[{\\\"address\\\":\\\"PbpVcjgYatnkKgveaeqhkeQBFwjqR7jKBR\\\",\\\"amount\\\":0.1}]\"")
                },
            }.ToString());
...
static UniValue createrawparttransaction(const JSONRPCRequest& request)
{
                    ...
                    {"outputs", RPCArg::Type::ARR, /* default */ "", "a json array with outputs (key-value pairs).",
                        {
                            {"", RPCArg::Type::OBJ, /* default */ "", "",
                                {
                                    {"address", RPCArg::Type::STR, /* default */ "", "The proinvestcoin address."},
    ...
    rawTx.nVersion = PROINVESTCOIN_TXN_VERSION;
...
static UniValue fundrawtransactionfrom(const JSONRPCRequest& request)
{
    ...
    tx.nVersion = PROINVESTCOIN_TXN_VERSION;
...
{ //  category              name                                actor (function)                argNames
  //  --------------------- ------------------------            -----------------------         ----------
    ...
    //sendpictopic // normal txn
    { "wallet",             "sendpictoblind",                  &sendpictoblind,               {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },
    { "wallet",             "sendpictoanon",                   &sendpictoanon,                {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },

    { "wallet",             "sendblindtopic",                  &sendblindtopic,               {"address","amount","comment","comment_to","subtractfeefromamount","narration"} },
    ...
    { "wallet",             "sendanontopic",                   &sendanontopic,                {"address","amount","comment","comment_to","subtractfeefromamount","narration","ringsize","inputs_per_sig"} },
