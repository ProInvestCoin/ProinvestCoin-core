...
std::shared_ptr<CWallet> LoadWallet(interfaces::Chain& chain, const WalletLocation& location, std::string& error, std::string& warning)
{
    ...
    if (fProInvestCoinMode) {
        if (!((CHDWallet*)wallet.get())->Initialise()) {
            error = "ProInvestCoin wallet initialise failed.";
            return nullptr;
        }
...
CAmount CWalletTx::GetAvailableCredit(interfaces::Chain::Lock& locked_chain, bool fUseCache, const isminefilter& filter) const
{
    ...
    for (unsigned int i = 0; i < tx->GetNumVOuts(); i++)
    {
        ...
        {
            nCredit += fProInvestCoinWallet ? pwallet->GetCredit(tx->vpout[i].get(), filter)
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, uint64_t wallet_creation_flags)
{
    ...
    std::shared_ptr<CWallet> walletInstance(fProInvestCoinMode
    ...
    if (gArgs.GetBoolArg("-upgradewallet", false) && !fProInvestCoinMode) {
    ...
    if (!fProInvestCoinMode) // Must rescan after hdwallet is loaded
...
int CMerkleTx::GetBlocksToMaturity(interfaces::Chain::Lock& locked_chain, const int *pdepth) const
{
    ...
    if (fProInvestCoinMode && (locked_chain.getHeight() < COINBASE_MATURITY * 2)) {
