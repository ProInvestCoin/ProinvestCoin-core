...
{
    // Create a message-only window to intercept WM_CLOSE events from proinvestcoind
 ...
 */
static const char* BITCOIN_PID_FILENAME = "proinvestcoin.pid";
...
{
#ifdef WIN32
    // Only proinvestcoind will create a hidden window to receive messages
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-rpccorsdomain=<domain>", "Allow JSON-RPC connections from specified domain (e.g. http://localhost:4200 or \"*\"). This needs to be set if you are using the ProInvestCoin GUI in a browser.", false, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/proinvestcoin/proinvestcoin-core>";
    const std::string URL_WEBSITE = "<https://proinvestcoin.io/>";
...
{
    fProInvestCoinMode = !gArgs.GetBoolArg("-btcmode", false); // qa tests
    if (!fProInvestCoinMode) {
        WITNESS_SCALE_FACTOR = WITNESS_SCALE_FACTOR_BTC;
        if (gArgs.GetBoolArg("-regtest", false)) {
            ResetParams(CBaseChainParams::REGTEST, fProInvestCoinMode);
        }
    }
    ...
    // TODO: Check pruning
    if (fPruneMode && fProInvestCoinMode)
    {
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fProInvestCoinMode) { // SMSG breaks functional tests with services flag, see version msg
#ifdef ENABLE_WALLET
