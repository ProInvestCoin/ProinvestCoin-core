...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, const CScriptWitness* witness, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (checker.IsProInvestCoinVersion())
    {
        assert(witness);
        if (scriptSig.size() != 0) {
    ...
    bool fIsP2SH = checker.IsProInvestCoinVersion() ? scriptPubKey.IsPayToScriptHashAny(checker.IsCoinStake()) : scriptPubKey.IsPayToScriptHash();
