...
WalletTx MakeWalletTx(interfaces::Chain::Lock& locked_chain, CWallet& wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.tx->IsProInvestCoinVersion()) {
...
public:
    ...
    {
        if (::IsProInvestCoinWallet(wallet.get())) {
            m_wallet_part = GetProInvestCoinWallet(wallet.get());
        }
    }
    ...
    {
        if (total_fee > 0 || ::IsProInvestCoinWallet(m_wallet.get())) {
    ...
    bool IsProInvestCoinWallet() override
    {
    ...
    CHDWallet *getProInvestCoinWallet() override
    {
        return m_wallet_part;
    }
