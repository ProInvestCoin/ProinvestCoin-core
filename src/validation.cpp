...
#if defined(NDEBUG)
# error "ProInvestCoin cannot be compiled without assertions."
#endif
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (txFrom) {
            ...
            if (txFrom->IsProInvestCoinVersion())
            {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!fProInvestCoinMode  // genesis coinbase is spendable when in ProInvestCoin mode
        && fIsGenesisBlock) {
        if (!fJustCheck)
    ...
    if (fProInvestCoinMode || VersionBitsState(pindex->pprev, consensus, Consensus::DEPLOYMENT_CSV, versionbitscache) == ThresholdState::ACTIVE) {
    ...
    blockundo.vtxundo.reserve(block.vtx.size() - (fProInvestCoinMode ? 0 : 1));
    ...
    if ((!fIsGenesisBlock || fProInvestCoinMode)
...
void UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            if (fProInvestCoinMode)
            {
                if (pindex->nVersion > PROINVESTCOIN_BLOCK_VERSION)
                    ++nUpgraded;
            } else
            {
...
{
    if (fProInvestCoinMode) return true;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (fProInvestCoinMode && pindexPrev) {
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        if (miSelf != mapBlockIndex.end()) {
            ...
            if (fProInvestCoinMode && !IsInitialBlockDownload() && state.nodeId >= 0
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (!pindex->pprev
            ...
            if (fProInvestCoinMode && !IsInitialBlockDownload()) {
