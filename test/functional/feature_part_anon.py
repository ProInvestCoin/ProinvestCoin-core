...
class AnonTest(ProInvestCoinTestFramework):
    ...
    def run_test(self):
        ...
        txnHash = nodes[0].sendpictoanon(sxAddrTo1_1, 1, '', '', False, 'node0 -> node1 p->a')
        txnHashes.append(txnHash)

        txnHash = nodes[0].sendpictoblind(sxAddrTo0_1, 1000, '', '', False, 'node0 -> node0 p->b')
        txnHashes.append(txnHash)
        ...
        for k in range(4):
            txnHash = nodes[0].sendpictoanon(sxAddrTo1_1, 10, '', '', False, 'node0 -> node1 p->a')
            txnHashes.append(txnHash)
        for k in range(10):
        ...
        ro = nodes[1].sendtypeto('anon', 'pic', outputs, 'comment_to', 'comment_from', 4, 32, True)
        assert(ro['bytes'] > 0)

        txnHash = nodes[1].sendtypeto('anon', 'pic', outputs)
        txnHashes = [txnHash,]
