...
ALL_SCRIPTS = EXTENDED_SCRIPTS + BASE_SCRIPTS + PROINVESTCOIN_SCRIPTS + INSIGHT_SCRIPTS
...
def main():
    ...
    parser.add_argument('--proinvestcoin', action='store_true', help='run ProInvestCoin specific tests')
    ...
    tmpdir = "%s/proinvestcoin_test_runner_₿_🏃_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    #    test_list += ALL_SCRIPTS
        ...
        if args.proinvestcoin:
            test_list += PROINVESTCOIN_SCRIPTS
        if args.insight:
    ...
    run_tests(
        ...
        runs_ci=args.ci,
        create_cache=(True if args.bitcoin or (not args.proinvestcoin and not args.insight) else False)
    )
...
def run_tests(test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, failfast=False, runs_ci=False, create_cache=True):
    ...
    try:
        if subprocess.check_output(["pidof", "proinvestcoind"]) is not None:
            print("%sWARNING!%s There is already a proinvestcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
