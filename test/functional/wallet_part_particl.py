...
from test_framework.test_proinvestcoin import ProInvestCoinTestFramework
...
class WalletProInvestCoinTest(ProInvestCoinTestFramework):
    ...
    def proinvestcoin_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/proinvestcoin-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-regtest'] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_tool_output(self, output, *args):
        p = self.proinvestcoin_wallet_process(*args)
    ...
    def run_test (self):
        ...
        ro = nodes[2].sendtypeto('pic', 'pic', outputs, 'comment', 'comment-to', 4, 32, True, coincontrol)
        ...
        self.log.info('Test proinvestcoin-wallet')
        out = textwrap.dedent('''\
            Wallet info
            ===========
            Encrypted: no
