#!/usr/bin/env python3
# Copyright (c) 2018 The ProInvestCoin Core developers
...
from test_framework.test_proinvestcoin import (
    ProInvestCoinTestFramework,
    isclose,
    getIndexAtProperty,
)
...
class USBDeviceTest(ProInvestCoinTestFramework):
    ...
    def run_test(self):
        ...
        # Check that proinvestcoin has been built with USB device enabled
        ...
        if not config["components"].getboolean("ENABLE_USBDEVICE"):
            raise SkipTest("proinvestcoind has not been built with usb device enabled.")
