ProInvestCoin Core version 0.16.0.1 is now available.
...
ProInvestCoin Core 0.16 introduces generic support for Hardware Signing Devices to provide key-signing capabilities. 
...
### Ledger Hardware Support Support
...
In order to facilitate deterministic generation of stealth addresses between ProInvestCoin Core and Signing Devices the method used to generate these addresses has been changed.
...
Note, Legacy Stealth Addresses are not supported on Hardware Signing Devices. If you have PIC/tPIC received on the Legacy Stealth Addresses then we suggest you send those PIC/tPIC to a non-Stealth Address before attempting to use with the Hardware Signing Devices.
...
ProInvestCoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
proinvestcoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile ProInvestCoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µPIC" in the GUI now also show the more colloquial term "parts", specified in BIP176.
...
### Renamed `scanchain` to `rescanblockchain` RPC
...
- proinvestcoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- proinvestcoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for proinvestcoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to proinvestcoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µPIC to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_proinvestcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` proinvestcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.proinvestcoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for proinvestcoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in proinvestcoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop proinvestcoind (OmeGak)
...
- #10529 `331352f` Improve proinvestcoind systemd service file (Flowdalic)
